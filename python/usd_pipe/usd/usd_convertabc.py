...
def GetFlattenedUsdData(filePath, populationMaskPaths):
    ...
    popMask = None if populationMaskPaths is None else Usd.StagePopulationMask()
    if popMask:
...
def main(args):
    ...
    if args.out:
        if os.path.isfile(args.out) and not os.access(args.out, os.W_OK):
            _Err(
                "usdcatpython: error: no write permission for existing output file '%s'"
                % (args.out)
            )
            return 1
        if len(args.inputFiles) != 1:
            _Err(
                "usdcatpython: error: must supply exactly one input file with -o/--out"
            )
            return 1
        ...
        if args.usdFormat:
            if ext != "usd":
                _Err(
                    "usdcatpython: error: use of --usdFormat requires output file end "
                    "with '.usd' extension"
                )
                return 1
        ...
        if Sdf.FileFormat.FindByExtension(ext) is None:
            _Err("usdcatpython: error: unknown output file extension '.%s'" % (ext))
            return 1
    # If --out was not specified, then --usdFormat must be unspecified or must
    # be 'usda'.
    elif args.usdFormat and args.usdFormat != "usda":
        _Err(
            "usdcatpython: error: can only write 'usda' format to stdout; specify an "
            "output file with -o/--out to write other formats"
        )
        return 1
    ...
    if args.populationMask:
        if not args.flatten:
            ...
            _Err("usdcatpython: error: --mask requires --flatten")
            return 1
    ...
    if args.layerMetadata:
        if args.flatten:
            ...
            _Err(
                "usdcatpython: error: --layerMetadata cannot be used "
                "together with --flatten"
            )
        if args.flattenLayerStack:
            # Cannot parse only metadata when flattening.
            _Err(
                "usdcatpython: error: --layerMetadata cannot be used "
                "together with --flattenLayerStack"
            )
    ...
    for inputFile in args.inputFiles:
        ...
        if args.out:
            try:
                ...
                _Err("Error exporting '%s' to '%s' - %s" % (inputFile, args.out, e))
                ...
                if os.path.isfile(args.out):
                    ...
                    try:
                        ...
                        _Err("Possibly corrupt output file renamed to %s" % (newName,))
                    except Exception as e:
                        _Err(
