...
def CreateModelStage(
    ...
    if variantSetName and not Tf.IsValidIdentifier(variantSetName):
        print(
            "variantSetName '%s' must be a valid identifier. Aborting." % variantSetName
        )
        return None
    ...
    if not Kind.Registry.IsA(kind, Kind.Tokens.model):
        print("kind '%s' is not a valid model kind, which must be one of:" % kind)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog=os.path.basename(sys.argv[0]), description=descr
    )
