...
from usd_pipe.io.scenexml import constant
...
def read_scenegraph(layout_file):
    """basic layout with no support (see SceneXml for minimun api)
...
#####
def read_manifest(scene_file_manifest):
    """List of scenegraphXML3D (and subclass) in defaultdict(list)
    Args:
        scene_file_manifest(str): a json manifest file
    Returns:
        defaultdict(list): dict of scene xml with key: camera etc with scenegraphXML3D
    """
    from usd_pipe.io.manifestJsonXml import ManifestJsonXml

    return ManifestJsonXml.scene_from_json(scene_file_manifest)


#####
def get_main_files_manifest(scene_file_manifest):
    """same as read_manifest but with only main files, scene layout, camera ...
    Args:
        scene_file_manifest(str): a json manifest file
    Returns:
        defaultdict(list): dict of scene xml with key: camera etc with
                           only scenename camera grass layout and scatter files.
    """
    from usd_pipe.io.manifestJsonXml import ManifestJsonXml

    return ManifestJsonXml.get_main_files_manifest(scene_file_manifest)
...
#####################################
class ScannerDefault(object):
    """
    IN PROGRESS: to export a basic blinn
    """
    ...
    def scan_material(cls, scenenode_or_klf, resolve=False):
        """entry point for material info for klf file or node with get_klf method
            ...
            # https://github.com/PixarAnimationStudios/USD/tree/release/pxr/usd/usdHydra
        Args:
        ...
        tmpdic = dict()
        final_comp = dict()
        matpath = "%s/materials" % cls
        if len(tmpdic) > 0 and matpath != "":
            final_comp.update({"external:%s" % matpath: tmpdic})
        return final_comp
...
class SceneBuilder(object):
    ...
    def load_and_process(self, ctx, in_scene, material, outdir, scanner, force):
        ...
        if result[SchemaScene.layout]:
            ...
            container_layout = SceneXml(constant.attrlist)
        ...
        if result[SchemaScene.camera]:
            container_camera = SceneXml(constant.attrlist)
        ...
        if result[SchemaScene.animation]:
            container_anim = SceneXml(constant.attrlist)
        ...
        if result[SchemaScene.crowds]:
            ...
            container_crowds = SceneXml(constant.attrlist)
            container_crowds.name = SchemaScene.crowds_label
            for crowds_bunch in split_crowds:
                container_bunch = SceneXml(constant.attrlist)
        ...
        if result[SchemaScene.scatter]:
            container_scatter = SceneXml(constant.attrlist)
    ...
    def set_mtl(cls, material_list, key_mat, helper, force=False):
        ...
        Args:
            material_list(tupple): SceneXml instance,  dict (mat)
